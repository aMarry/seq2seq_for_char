...
def extract_character_vocab(data):
    ...
    构造映射表
    '''
    special_words = ['<PAD>', '<UNK>', '<GO>',  '<EOS>'] #<GO>起始,<EOS> 终止, <UNK>不在映射表中
...
target_int[:20] # target比source多一个终止符 所以多一位
...
def get_encoder_layer(input_data, rnn_size, num_layers,
    ...
    - source_vocab_size: 源数据的词典大小 不重复的数据
    - encoding_embedding_size: embedding的大小 维度
    '''
    # Encoder embedding
...
def decoding_layer(target_letter_to_int, decoding_embedding_size, num_layers, rnn_size,
    ...
    decoder_embed_input = tf.nn.embedding_lookup(decoder_embeddings, decoder_input) #映射完的 查找
    ...
    # 与training共享参数 scope相同 reuse=True 共享参数
